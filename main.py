...
if __name__ == "__main__":
    # Flags to display data.
    reports = False

    # Display the timestamps of the signals.
    timestamps = False

    # Display the heatmaps of the signals.
    heatmaps = False

    cmps = False

    # Which experiments to show.
    showflags = [False, False, False, False]

    # Load the data and analyze it.
    base = SingleExperimentData.load( 'data/damage/dmga/CE23Test103.dat' )
    base.analyze()
    base.rename("BASE")
    dmga = SingleExperimentData.load( 'data/damage/dmga/CE23Test104.dat' )
    dmga.analyze()
    dmga.rename("DMGA")
    dmgb = SingleExperimentData.load( 'data/damage/dmga/CE23Test105.dat' )
    dmgb.analyze()
    dmgb.rename("DMGB")
    dmgc = SingleExperimentData.load( 'data/damage/dmga/CE23Test106.dat' )
    dmgc.analyze()
    dmgc.rename("DMGC")

    # Compare both experiments.
    ca = base.compare( dmga )
    cb = base.compare( dmgb )
    cc = base.compare( dmgc )

    comparisons = [ca, cb, cc]

    dmgtype = ["5 cm from 6", "1 cm from 6", "1 cm from 1"]

    if cmps:
        for i, comp in enumerate( comparisons ):
            # Create the figure.
            fig = go.Figure()
            fig.update_layout(title=f"Delay for damage {dmgtype[i]}")

            # Create the list of pairs in the comparison.
            pairs = comp.pairs()

            for pair in comp.pairs():
                s = sorted(pair, key=lambda p : p.srcfreq)
                #series = pd.Series( [p.ssr - p.ssl for p in s] )
                series = pd.Series( [p.delay       for p in s] )
                freqs  = pd.Series( [p.sfr         for p in s] )

                fig.add_scatter( x=freqs, y=series, name=f"Delay ACT{pair[0].act} - SEN{pair[0].sen}", mode='markers' )

            fig.show()

    # Process the signals.
    for show, exp in zip( showflags, [base, dmga, dmgb, dmgc] ):
        if not show:
            continue

        for signal in exp.signals:
            # Show the report of the signal.
            if reports:
                signal.report()

            # Show the signal start and end markers.
            if timestamps:
                # Create the figure.
                fig = go.Figure()
                ...
                # Set the title
                fig.update_layout(
                    title=dict(text=f"[{exp.name}] - Sensor {signal.sensor.id} data [ACT {signal.actuator.id}]", font=dict(size=25), yref='paper')
                )

                # Add all the graphs.
                fig.add_scatter( y=pd.Series( signal.sen.flatten() ), name='Sensor signal [filter]' )
                fig.add_scatter( y=pd.Series( signal.act.flatten() ), name='Source signal [smoothed]' )
                fig.add_scatter( y=pd.Series( signal.raw.flatten() ), name='Sensor signal [raw]' )
                fig.add_vline( signal.endSource, line={'color': "#000000"} )
                fig.add_vline( signal.startSignal, line={'color': "#FF00FF"} )
                fig.add_vline( signal.sigmax, line={'color': "#00FF00"} )

                fig.show()

            #Show the signal heatmap.
            if heatmaps:
                # Create the heatmap.
                fig = go.Figure( go.Heatmap( x=signal.fftT, y=np.divide( signal.fftF, 1000 ), z=signal.fftZ ) )

                # Set the title
                fig.update_layout(
                    title=dict(text=f"[{exp.name}] - Sensor {signal.sensor.id} heatmap [ACT {signal.actuator.id}]", font=dict(size=25), yref='paper'),
                    xaxis_title="T [us]",
                    yaxis_title="Frequency [kHz]"
                )
